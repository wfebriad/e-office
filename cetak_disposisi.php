<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        <div class="container">
            <div id="colres">
                <div class="disp">';
                    $query2 = mysqli_query($config, "SELECT kntr_pst, nama, kementerian, alamat, logo FROM tbl_instansi");
                    list($kntr_pst, $nama, $kementerian, $alamat, $logo) = mysqli_fetch_array($query2);
                    if(!empty($logo)){
                    ...
                    if(!empty($kementerian)){
                        echo '<h6 class="up">'.$kementerian.'</h6>';
                    } else {
                        echo '<h6 class="up">KEMENTRIAN PERHUBUNGAN</h6>';
                    }
                    if(!empty($kntr_pst)){
                        echo '<h6 class="up" id="nama">'.$kntr_pst.'</h6><br/>';
                    } else {
                        echo '<h6 class="up" id="nama">DIREKTORAT JENDERAL PERHUBUNGAN LAUT</h6><br/>';
                    }
                    if(!empty($nama)){
                        echo '<h5 class="kementerian">'.$nama.'</h5>';
                    } else {
                        echo '<h5 class="kementerian">DISTRIK NAVIGASI KELAS 1 TAMJUNGPINANG</h5>';
                    }
                    if(!empty($alamat)){
                        echo '<span id="alamat">'.$alamat.'</span>';
                    } else {
                        echo '<span id="alamat">Jl. Sei Walang Kijang Kepulauan Riau 29151</span>';
                    }
                    echo '
                </div>
            ...
            <div id="lead">
                <p>Kepala Distrik Navigasi Kelas I Tanjungpinang</p>
                <div style="height: 50px;"></div>';
                $query = mysqli_query($config, "SELECT kep_inst, nip FROM tbl_instansi");
                list($kep_inst,$nip) = mysqli_fetch_array($query);
                if(!empty($kep_inst)){
                    echo '<p class="lead">'.$kep_inst.'</p>';
                } else {
                    echo '<p class="lead">Kepala Distrik Navigasi Kelas I Tanjungpinang</p>';
                }
                if(!empty($nip)){
                    echo '<p>NIP. '.$nip.'</p>';
                } else {
